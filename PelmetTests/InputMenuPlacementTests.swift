...
struct InputMenuPlacementTests {
    ...
    @Test func assignedInputMenuQueuesBeforeAnyRegistrationPass() {
        ...
        #expect(AppState.relaunchAdoptionKeys(for: bundle, model: model) == [input.sectionKey])
        #expect(AppState.relaunchAdoptionKeys(for: bundle, model: SectionModel(knownBundles: ["com.example.Other"])).isEmpty)
    }
    ...
    @Test func existingInstallFoldsSystemHostsWithoutRouting() {
        ...
        // Folded hosts stay eligible for relaunch adoption once assigned.
    ...
    @Test func unmanagedAppleAndOwnItemsStayExcluded() {
        ...
        for bundle in [PelmetBundle.agentID, PelmetBundle.mainID] {
            ...
            #expect(AppState.relaunchAdoptionKeys(for: bundle, model: model).isEmpty)
        }
        #expect(AppState.relaunchAdoptionKeys(for: bundle, model: SectionModel()).isEmpty)
    }
    ...
    @Test func unnamedAppleHelperIsPlacedLikeAnyApp() {
        ...
        #expect(AppState.relaunchAdoptionKeys(for: "com.apple.weather.menu", model: model) == [weather.sectionKey])
    }
