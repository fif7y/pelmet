...
| 51 | Clock click: delayed shading under Notification Center (beta.2) | Exit: plain replayed click, no blink. Entrance: cover to the display edge; the click's own picture with the panel's measured shade wiped in from the right on the filmed trajectory, 130ms after the click is up, 110pt soft edge; bare cover beneath until the lift. Idle and finished pictures are only used in the panel state they were taken in. Gab 18:07: right. | **for beta.3**, close in its comment |
