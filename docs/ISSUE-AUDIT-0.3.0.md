...
| 51 | Clock click: delayed shading under Notification Center (beta.2) | The panel is a layer-21 window under the glass; a still cannot follow its slide, and no gradient matched it. Final: exit is a plain replayed click (no blink, no cover); entrance is two stills, bare until the press + 120ms, then the bar as it looks under the open panel (taken once, kept while the bar holds); cover ends at the leftmost live icon; NC out of the backdrop signature. Gab: exit perfect, entrance to confirm (2026-09-22 16:21). | **for beta.3**, close in its comment |
