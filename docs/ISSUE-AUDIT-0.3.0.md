...
| 51 | Clock click: delayed shading under Notification Center (beta.2) | Exit: plain replayed click, no blink. Entrance: cover to the display edge; the click's own picture with the panel's measured shade wiped in from the right on the filmed trajectory, 130ms after the click is up, 110pt soft edge; bare cover beneath until the lift. Idle and finished pictures are only used in the panel state they were taken in. Gab 18:07: right. | **shipped in beta.3, closed 2026-09-22** |
| 52 | A bar picture left on screen for four hours (beta.5, robinhur) | `ec4582e`: the shaded-picture swap could run after the blink was dismissed and raised a picture nothing held; the safety timer held the overlay weakly and had nothing to fade once it was freed. `BlinkCover.dismissed` refuses late swaps, the timer holds strongly and logs its fire, covers carry ids in the `ghost:` lines, a press during a blink is ignored. Live 2026-09-23 09:19: 12 presses in 1.5s, six covers, `cover #1 #2 #3 #4 #5 #6 down`, 0 Pelmet windows left at `.statusBar` level. | **shipped in beta.7, closed 2026-09-23** |
| 53 | NC shortcut opens the panel then closes it, four displays (robinhur) | `ec4582e`: verify poll 0.3→0.6s (ends when the panel shows), shortcut presses the clock on the pointer's display (same inset from the right edge, hit-tested), no click fallback for the shortcut. Escape and an AX press both leave an open panel alone (probed 09:11/09:14), closing stays a replayed click. Live 09:20: pointer on the BenQ → `press at 5170,19`, open in one press, closed by the next. | **shipped in beta.7, closed 2026-09-23**, robinhur asked to confirm on four displays |
| 54 | Shortcut recorder ignores the click (Korean, narrow row) | `ec4582e`: the chip reserves its widest label's width, so recording cannot flip `SettingRow`'s ViewThatFits to a fresh recorder. Live 09:23 in Korean at 720pt: the NC row's chip reads `단축키를 누르세요…` on click. | **shipped in beta.7, closed 2026-09-23** |
| 55 | A shortcut macOS owns is accepted and never fires | `ec4582e`: `SystemShortcuts.owns` reads the enabled `com.apple.symbolichotkeys` entries; the row's caption says macOS uses it and where to turn it off. Standalone check finds ⌃↑ (Mission Control) on Gab's Mac; entries absent from the plist (never customised) are not seen. | **shipped in beta.7, closed 2026-09-23** |
| PR 57 | Black menu bar under the notch (sbw125) | Held 2026-09-23 for after 0.3.0: the panel (level 24) is Pelmet's own window, so the captures leave it out and every cover would flash the wallpaper tint over a black bar; paints every display; loose defaults key, bare Toggle in the editor tab. Rework needs: capture aware of the panel, `SettingsStore`, Displays row, notched-only or named honestly. | **held, comment posted** |
