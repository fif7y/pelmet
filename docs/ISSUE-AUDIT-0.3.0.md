...
| 51 | Clock click: delayed shading under Notification Center (beta.2) | Exit: plain replayed click, no blink. Entrance: cover to the display edge; a still of the bar under the open panel (per bar state, stand-in = bare + measured shade on the first click) wiped in from the right on the panel's filmed trajectory, 105ms after the click is up, 110pt soft edge. Every picture (idle, finished strip, under-panel) is stamped with the panel state and only used in that state. Gab 17:50: "mostly there", one glitch left (panel open + expand), fixed `7efc65e`. | **for beta.3**, close in its comment |
