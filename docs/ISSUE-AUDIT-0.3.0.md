...
| 45 | Clock relay lights the recording dot, pointer jumps | By design: the blink cover captures the strip (lights the dot when no fresh picture exists) and replays the click at the clock's centre with a cursor warp when it landed on the dot. | not addressed. Checked 2026-09-21: the pointer half already ships (a dot-zone click is an AX press, no mouse event, 4/4 opened NC today; the warp-back covers the rare replay fallback). The dot half is not cheap: the blink covers leftmost-icon..clock, wider than the precaptured empty strip, so a picture reuse is a new cover path, not a swap. |
