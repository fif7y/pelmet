...
**After 4.5 (11:22, punched cover + cut-out memoized on their inputs, cover windows pooled per display, finished picture taken as soon as the cover lifts):** cover **5–14ms**, reveal settled **15–26ms** warm; the finished picture now lands ~150ms after every lift (sub-second reveals included), so the conceal path no longer captures live. A first pool that re-framed any spare window onto any display cost 0.5–1s per reveal (backing-scale change on the move) — keyed per display it measured free (cover 5–14ms) but showed pictures half-drawn on Gab's bar (stale backing contents of the reused window) — DROPPED for good; cover windows are created per reveal, 14–39ms, and that is the floor here. Open, pre-existing: the capture indicator shifts the cluster ~16pt for ~3s after any capture; a finished picture taken lit is dropped (`chevron moved since the picture`) by a reveal that happens unlit — cadence-dependent. FIXED 11:30: with a cover present the picture is cut out against it at its own geometry (the wallpaper is fixed in screen space) and the icons floated shifted by the chevron delta — `finished: chevron moved … icons cut out and shifted 16pt`, settled 41–45ms, same as unshifted reveals.

**After 4.8/4.7 (11:42, companion glyph swaps deduped by input key, running-bundle set pushed from the app's KVO):** plan 0–1ms (was 1–19ms), everything else unchanged. Trap hit on the way: `NSWorkspace.observe(\.runningApplications)` delivers the *change* in `newValue` (inserted/removed apps), not the array; pushed as the running set it pruned every concealed entry as "quit app" and swapped a 2-bundle allowlist (11:38). Read `NSWorkspace.shared.runningApplications` in the handler. `restSnapshot` now also refuses an empty mirror.
