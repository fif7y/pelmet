...
- **M4 release 0.3.0** (prep landed 2026-09-21): `OrderEdits` seed once at the first fully framed snapshot after the upgrade — a section whose stored drawing disagrees with the bar becomes a pending edit (Apply reproduces it, Discard keeps the bar); the lit Apply button is the "Tidy now" offer, no dialog. Release notes `docs/release-notes/v0.3.0.md`, README editor copy. Not in 0.3.0: the "Keep sections grouped" automatic pass (§above) — Apply is the only move; an editor drop between sections shows the not-in-place badge until Apply. Marketing assets stand: the cover keeps the reveal's look.
